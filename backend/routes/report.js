...
import { addLogoToWorksheet } from '../utils/excelWithLogo.js';
  ...
  const cumulativeAbsences = await Promise.all(
    studentsInClass.map(async (student) => {
      try {
        ...
        // OD is considered as present
        const presentCount = studentAttendanceRecords.filter(r => {
          const status = r.status && r.status.toLowerCase();
          return status === 'present' || status === 'od';
        }).length;
        const absentCount = studentAttendanceRecords.filter(r => {
          const status = r.status && r.status.toLowerCase();
...
router.get('/absentees/export/excel', async (req, res) => {
  try {
    ...
    // Determine number of columns for proper merging
    const headers = ['Roll No', 'Student Name', 'Date', 'Status', 'Reason'];
    const numColumns = headers.length;

    // Add professional PMC header with logo
    await addLogoToWorksheet(worksheet, null, {
      title: 'Absentees Report',
      department: currentUser.department,
      showDate: true
    }, numColumns);

    // Add class information (row 8 onwards after header)
    let currentRow = 8;
    const infoRow = worksheet.getRow(currentRow);
    infoRow.getCell(1).value = `Class: ${classToQuery}`;
    infoRow.getCell(1).font = { name: 'Calibri', size: 11 };
    const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
    worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
    infoRow.getCell(1).alignment = { horizontal: 'center' };
    currentRow++;
    
    if (date) {
      const dateInfoRow = worksheet.getRow(currentRow);
      dateInfoRow.getCell(1).value = `Date: ${date}`;
      dateInfoRow.getCell(1).font = { name: 'Calibri', size: 11 };
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
    worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
      dateInfoRow.getCell(1).alignment = { horizontal: 'center' };
      currentRow++;
    } else if (startDate && endDate) {
      const dateRangeRow = worksheet.getRow(currentRow);
      dateRangeRow.getCell(1).value = `Date Range: ${startDate} to ${endDate}`;
      dateRangeRow.getCell(1).font = { name: 'Calibri', size: 11 };
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
    worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
    ...
    headers.forEach((header, index) => {
      ...
      cell.fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF1E3A8A' } // Dark navy background
      };
    ...
    if (!attendanceMarked) {
      ...
      noDataRow.getCell(1).value = '⚠️ Attendance not taken for this date.';
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
      worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
      noDataRow.getCell(1).alignment = { horizontal: 'center' };
      currentRow++;
    } else if (absenteeRecords.length === 0) {
      const allPresentRow = worksheet.getRow(currentRow);
      allPresentRow.getCell(1).value = '✅ All students were present on this date.';
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
      worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
      allPresentRow.getCell(1).alignment = { horizontal: 'center' };
      currentRow++;
    } else {
      // Sort absentees by roll number
      const sortedAbsentees = absenteeRecords.map(record => {
      ...
      sortedAbsentees.forEach(absentee => {
        ...
        const values = [absentee.rollNo, absentee.name, absentee.date, absentee.status, absentee.reason];
        ...
        headers.forEach((header, index) => {
          ...
          if (headerLower.includes('date') || headerLower.includes('roll no') || headerLower.includes('no')) {
            cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
          } else {
      ...
      summaryRow.getCell(4).value = 'Total Absentees:';
      summaryRow.getCell(5).value = sortedAbsentees.length;
      summaryRow.getCell(4).font = { name: 'Calibri', size: 11, bold: true };
      summaryRow.getCell(5).font = { name: 'Calibri', size: 11, bold: true };
      currentRow++;
    }
...
router.get('/enhanced-absentees/export/excel', async (req, res) => {
  try {
    ...
    // Add professional PMC header with logo
    await addLogoToWorksheet(worksheet, null, {
      title: 'Enhanced Absentees Report',
      department: currentUser.department,
      showDate: true
    }, numColumns);
    ...
    // Add class information (row 8 onwards after header)
    let currentRow = 8;
    const classInfoRow = worksheet.getRow(currentRow);
    classInfoRow.getCell(1).value = `Class: ${batch}, ${year}, Sem ${semester}${section ? `, Section ${section}` : ''}`;
    classInfoRow.getCell(1).font = { name: 'Calibri', size: 11 };
    const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
    worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
    classInfoRow.getCell(1).alignment = { horizontal: 'center' };
    currentRow++;
    ...
    if (!attendanceMarked) {
      const noDataRow = worksheet.getRow(currentRow);
      noDataRow.getCell(1).value = '⚠️ No attendance records found for this date/class.';
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
      worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
      noDataRow.getCell(1).alignment = { horizontal: 'center' };
      currentRow++;
      ...
      const allPresentRow = worksheet.getRow(currentRow);
      allPresentRow.getCell(1).value = '✅ No absentees found for the selected period.';
      const lastColumn = String.fromCharCode(64 + Math.min(numColumns, 26));
      worksheet.mergeCells(`A${currentRow}:${lastColumn}${currentRow}`);
      allPresentRow.getCell(1).alignment = { horizontal: 'center' };
      currentRow++;
      ...
      cumulativeAbsences.forEach((student, index) => {
        ...
        // Style data rows professionally with proper alignment
        headers.forEach((header, colIndex) => {
          const cell = dataRow.getCell(colIndex + 1);
          const value = colIndex === 0 ? index + 1 :
                       colIndex === 1 ? yearText :
                       colIndex === 2 ? student.rollNumber :
                       colIndex === 3 ? student.name :
                       colIndex === 4 ? (student.mobile || 'N/A') :
                       colIndex === 5 ? student.totalAbsentDays :
                       colIndex === 6 ? (student.reason || 'No reason provided') :
          ...
          // Apply appropriate alignment
          const headerLower = header.toLowerCase();
          if (headerLower.includes('s.no') || headerLower.includes('no.') || headerLower.includes('absent')) {
            cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
          } else if (headerLower.includes('number') || headerLower.includes('phone')) {
      ...
      summaryRow1.getCell(6).value = 'Total Absentees:';
      summaryRow1.getCell(7).value = cumulativeAbsences.length;
      summaryRow1.getCell(6).font = { name: 'Calibri', size: 11, bold: true };
      summaryRow1.getCell(7).font = { name: 'Calibri', size: 11, bold: true };
      currentRow++;

      const summaryRow2 = worksheet.getRow(currentRow);
      summaryRow2.getCell(6).value = 'Total Students:';
      summaryRow2.getCell(7).value = studentsInClass.length;
      summaryRow2.getCell(6).font = { name: 'Calibri', size: 11, bold: true };
      summaryRow2.getCell(7).font = { name: 'Calibri', size: 11, bold: true };
      currentRow++;
